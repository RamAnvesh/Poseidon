...
import com.flipkart.phantom.task.spi.AbstractTaskResult;
 ...
 */
public abstract class AbstractFutureTaskResultToDomainObjectPromiseWrapper<F extends Future<TaskResult>, DomainObject> implements Promise<DomainObject> {

    final List<F> futureList = new ArrayList<>();
    ResponseMerger<DomainObject> responseMerger;
    final boolean throwOriginalException;

    AbstractFutureTaskResultToDomainObjectPromiseWrapper(F future) {
        this(future, false);
    }

    AbstractFutureTaskResultToDomainObjectPromiseWrapper(F future, boolean throwOriginalException) {
        futureList.add(future);
        this.throwOriginalException = throwOriginalException;
    }

    AbstractFutureTaskResultToDomainObjectPromiseWrapper(ResponseMerger<DomainObject> responseMerger) {
        this(responseMerger, false);
    }

    AbstractFutureTaskResultToDomainObjectPromiseWrapper(ResponseMerger<DomainObject> responseMerger, boolean throwOriginalException) {
    ...
    public void addFutureForTask(F future) {
        futureList.add(future);
    }

    public void addFutureForTask(List<? extends F> future) {
        futureList.addAll(future);
    }

    public List<F> getFutureList() {
        return futureList;
    }
